...
def main():
    ...
    close(settings)
    settings = launch(3, '^WiiDesk Settings$')
    run('xdotool', 'key', 'Down', 'Down', 'Left', 'Return')
    wait_for(lambda: 'idle_lock_seconds=60\n' in config.read_text(), 'idle timeout saved from Settings')
    close(settings)
    settings = launch(3, '^WiiDesk Settings$')
    run('xdotool', 'key', 'Down', 'Down', 'Right', 'Return')
    wait_for(lambda: 'idle_lock_seconds=300\n' in config.read_text(), 'idle timeout reloads and restores')
    close(settings)
